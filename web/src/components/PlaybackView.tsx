...
export function PlaybackView({ candidate, onBack }: PlaybackViewProps) {
  ...
  useEffect(() => {
    ...
    if (isHls && !nativeHls && Hls.isSupported()) {
      ...
      setError('This browser has no native HLS or Media Source support for this stream.')
    }
  ...
  return (
    <div className="playback-page">
      <div className="playback-header"><button className="button button-quiet" onClick={onBack}><Icon name="back" size={16} />Back to event</button><span className="playback-source-label">{candidate.sourceKind} · {candidate.title}</span></div>
      ...
      <div className="playback-diagnostics"><span className="diagnostic-label"><Icon name="shield" size={14} />Browser diagnostics</span><span>{candidate.quality.resolution || 'Quality not reported'}</span><span>{candidate.headers ? 'Addon headers supplied' : 'Direct request'}</span><span>{Math.round(performance.now() - startedAt)} ms session</span></div>
    </div>
  )
}
