...
export function LiveTvPage({ config, onPlay }: LiveTvPageProps) {
  ...
  return <div className="iptv-page"><aside className="iptv-sidebar" data-focus-zone="categories"><span className="panel-label">LIVE TV</span><nav data-focus-column>{categories.map((item) => <button className={item === category ? 'is-active' : ''} key={item} onClick={() => setCategory(item)}>{item}</button>)}</nav><a className="button button-quiet" href={routeHash({ page: 'home' })}>‹ Sports</a></aside><section className="iptv-content" data-focus-zone="content"><header data-focus-row><div><h2>{category === 'All' ? 'All channels' : category}</h2><p>{visible.length} channels · {channels.length} total</p></div><label className="iptv-search"><Icon name="search" size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search channels" aria-label="Search channels" /></label></header>
    {!config.portalUrl || !config.macAddress ? <div className="iptv-empty-wrapper"><EmptyState title="Live TV needs your provider" body="Add the portal URL and MAC address supplied by your authorized IPTV provider." action={<a className="button button-primary" href={routeHash({ page: 'settings', section: 'sources' })}><Icon name="settings" size={16} />Open settings</a>} /></div> :
     ...
     <div className="channel-grid" data-focus-grid data-focus-columns="4">{error && <div className="notice notice-warning" style={{ gridColumn: '1 / -1' }}><Icon name="alert" size={16} /><span>{error}</span><button className="button button-quiet button-small" onClick={() => void load()}>Retry</button></div>}{visible.map((channel) => <button key={channel.id} className="channel-card" onFocus={() => void loadGuide(channel)} onClick={() => void open(channel)}><b>{channel.number}</b><div className="channel-logo">{channel.logoUrl ? <img src={channel.logoUrl} alt="" loading="lazy" /> : <span>{channel.name.slice(0, 3)}</span>}</div><div className="channel-copy"><strong>{channel.name}</strong><small>{opening === channel.id ? 'Connecting…' : channel.guide?.now?.title ?? channel.category}</small></div></button>)}</div>}
  </section></div>
}
