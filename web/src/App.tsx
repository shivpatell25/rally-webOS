...
function OnboardingPage() {
  const steps = [
    ['01', 'Choose your sports', 'Arrange leagues and favorite teams so Rally promotes the games that matter to you.'],
    ['02', 'Connect your sources', 'Add only the IPTV portals and Stremio addons you are authorized to use.'],
    ['03', 'Watch your way', 'Use source selection, Game View, current highlights, and Multi-View from one remote-first interface.'],
  ]
  return <div className="onboarding-page"><Brand /><h1>Sports, kept simple.</h1><p>Live games, verified sources, highlights, and the teams you follow—built for the biggest screen in your home.</p><div className="onboarding-cards">{steps.map(([number, title, description]) => <article key={number}><span>{number}</span><h2>{title}</h2><p>{description}</p></article>)}</div><a className="button button-primary" data-initial-focus="true" href={routeHash({ page: 'settings', section: 'sources' })}>Continue to setup</a><small>Rally includes no television service or streams.</small></div>
}
