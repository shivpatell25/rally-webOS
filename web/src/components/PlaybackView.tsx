...
export function PlaybackView({ candidate, preferences, event, alternatives, onSelect, onBack, onMultiView }: PlaybackViewProps) {
  ...
  return <div className={`playback-page ${preferences.scoreSaver ? 'score-saver-enabled' : ''}`} onMouseMove={() => setControlsVisible(true)}>
    ...
    <div className={`player-chrome ${controlsVisible ? 'is-visible' : ''}`} data-focus-scope={sourcePicker ? 'modal' : undefined}>
      <header className="playback-header"><button className="button button-quiet" onClick={closeOrBack}><Icon name="back" size={16} />Back</button><div><strong>{event?.name ?? candidate.title}</strong><span>{candidate.sourceKind} · {candidate.title}</span></div><div data-focus-row>{event && <button className={`button button-quiet ${gameView ? 'is-selected' : ''}`} onClick={() => { setSourcePicker(false); setGameView(!gameView) }}>Game View</button>}{onMultiView && <button className="button button-quiet" onClick={onMultiView}>Multi-View</button>}</div></header>
      <div className="player-control-bar" data-focus-row><button className="button button-primary" onClick={() => void player.current?.play()}><Icon name="play" size={16} />Play</button><button className="button button-quiet" onClick={() => player.current?.pause()}>Pause</button><button className="button button-quiet" onClick={() => player.current?.restart()}>Restart</button><button className="button button-quiet" onClick={() => player.current?.seekLive()}>Go live</button><button className="button button-quiet" onClick={() => { setGameView(false); setSourcePicker(!sourcePicker) }}>Sources ({alternatives.length || 1})</button><span>{status === 'playing' ? 'LIVE' : statusDetail ?? status}</span></div>
